...
$sql = "SELECT * FROM `users` WHERE `users`.`user_status` = 1";

$sqlcnt = "SELECT COUNT(*) FROM `users` WHERE `users`.`user_status` = 1";
...
function nestedData_work_list($row)
{
    ...
    $button_edit = $class_datatable->button_edit_user();
    $button_delete = $class_datatable->button_delete_user();
